...
from core.ui import print_welcome, print_user_prompt, print_error, print_info, print_plan_for_approval, print_plan_approval_options
from core.session import load_session, get_latest_session_id
from core.memory import list_memories
from core.skills import discover_skills, resolve_skill_prompt, get_skill_by_name, execute_skill


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="agent-template",
        description="Agent Template — a minimal coding agent",
        add_help=False,
    )
...
async def run_repl(agent: Agent) -> None:
    ...
    while True:
        ...
        try:
            try:
                # Use sys.stdin.buffer.readline() to get the raw byte stream
                raw_input = sys.stdin.buffer.readline()
                if not raw_input:
                    break
                # Use errors='replace' to prevent crashing on invalid UTF-8 sequences
                line = raw_input.decode('utf-8', errors='replace').strip()
            except Exception:
                # Fallback to an empty string to keep the loop running
                line = ""
        ...
        if inp == "/skills":
            ...
            if not skills:
                print_info("No skills found. Add skills under skills/ directory.")
            else:
...
def main() -> None:
    ...
    if args.help:
        print("""
Usage: agent-template [options] [prompt]
...
Options:
  ...
  --thinking          Enable extended thinking (Anthropic only)
    --model, -m         Model to use (default: claude-opus-4-6, or AGENT_TEMPLATE_MODEL/MINI_CLAUDE_MODEL env)
...
Examples:
    agent-template "fix the bug in src/app.ts"
    agent-template --yolo "run all tests and fix failures"
    agent-template --plan "how would you refactor this?"
    agent-template --max-cost 0.50 --max-turns 20 "implement feature X"
    OPENAI_API_KEY=sk-xxx agent-template --api-base https://aihubmix.com/v1 --model gpt-4o "hello"
    agent-template --resume
    agent-template  # starts interactive REPL
""")
        sys.exit(0)

    permission_mode = _resolve_permission_mode(args)
    model = args.model or os.environ.get("AGENT_TEMPLATE_MODEL") or os.environ.get("MINI_CLAUDE_MODEL", "claude-opus-4-6")
