...
from .skills import build_skill_payload
...
class Agent:
    def __init__(
        ...
        *,
        ...
        self.tools = custom_tools or get_tool_definitions()
        ...
        # Custom emit function (Web server routes output to WebSocket)
        self._custom_emit_fn: Callable[[str], None] | None = None

        # Progress callback for tools that emit intermediate steps
        self._progress_callback: Callable[[str, dict], None] | None = None

        # Session ID for per-chat output isolation (set by WebAgent)
        self._session_id: str = ""

        # Read-before-edit: track file read timestamps (absolutePath → mtime)
        self._read_file_state: dict[str, float] = {}

        # Structured skill metadata payload sent alongside model calls.
        self._skill_payload = build_skill_payload()
    ...
    @staticmethod
    def _call_supports_extra_body(callable_obj: Any) -> bool:
        try:
            return "extra_body" in inspect.signature(callable_obj).parameters
    ...
    def set_emit_fn(self, fn: Callable[[str], None] | None) -> None:
        self._custom_emit_fn = fn

    def set_progress_callback(self, fn: Callable[[str, dict], None] | None) -> None:
        """Set a callback for tools that produce intermediate progress steps (e.g. literature pipeline)."""
        self._progress_callback = fn

    async def generate_title(self, message: str) -> str:
        """Generate a concise conversation title from the user's first message using the LLM."""
        prompt = "Generate a very concise conversation title (max 15 characters, in Chinese if the message is in Chinese). Output ONLY the title, no quotes, no punctuation, no explanation."
        try:
            if self._anthropic_client:
                resp = await self._anthropic_client.messages.create(
                    model=self.model, max_tokens=100, system=prompt,
                    messages=[{"role": "user", "content": message}],
                )
                title = "".join(b.text for b in resp.content if b.type == "text").strip().strip("\"'")
                logger.info("generate_title (anthropic): model=%s title=%s", self.model, title)
                if title:
                    return title
                ...
                logger.info("generate_title (openai): model=%s title=%s", self.model, title)
                if title:
                    return title
            else:
                logger.warning("generate_title: no API client available (both _anthropic_client and _openai_client are None)")
        except Exception as e:
            logger.warning("generate_title: API call failed: %s", e)
        # Fallback: truncate first meaningful segment
        logger.warning("generate_title: falling back to truncation for message=%s", message[:60])
        for sep in ('。', '？', '！', '?', '!', '.', '\n', '，', ','):
            idx = message.find(sep)
            if 5 < idx <= 40:
                title = message[:idx].strip()[:20]
                logger.warning("generate_title: fallback title=%s", title)
                return title
    ...
    def _emit_text(self, text: str) -> None:
        if self._custom_emit_fn is not None:
            self._custom_emit_fn(text)
        elif self._output_buffer is not None:
    ...
    def _auto_save(self) -> None:
        try:
            sid = self._session_id or self.session_id
            save_session(sid, {
                "metadata": {
                    "id": sid,
                    "model": self.model,
    ...
    async def _execute_tool_call(self, name: str, inp: dict) -> str:
        ...
        # Inject progress callback + session ID for literature pipeline tools
        # so intermediate steps can be sent to the UI and outputs go to a
        # per-chat subdirectory. Covers v1 (lit_dimension_pipeline, literature_agent)
        # and v2 (literature_plan, literature_search, literature_generate_report).
        if self._progress_callback and name in (
            "lit_dimension_pipeline", "literature_agent",
            "literature_plan", "literature_search", "literature_db_search",
            "literature_generate_report", "literature_confirm",
            "literature_quick_search", "literature_analyze_papers",
            "literature_specify_paper",
            "literature_similar_search"
        ):
            extra = {"_progress_callback": self._progress_callback}
            if self._session_id:
                extra["_session_id"] = self._session_id
            inp = {**inp, **extra}
            logger.info("Injected _progress_callback for %s, session=%s", name, self._session_id)
...
IMPORTANT: When your plan is complete, you MUST call exit_plan_mode. Do NOT ask the user to approve — exit_plan_mode handles that."""
    ...
    async def _call_anthropic_stream(self, on_tool_block_complete=None):
        ...
        async def _do():
            ...
            # If supported by SDK/backend, pass skills as a dedicated payload
            # instead of burying everything inside natural-language prompts.
            if self._skill_payload and self._call_supports_extra_body(self._anthropic_client.messages.stream):
                create_params["extra_body"] = {"skills": self._skill_payload}
    ...
    async def _chat_openai(self, user_message: str) -> None:
        ...
        while True:
            ...
            # Strip fields that some providers return in responses but reject
            # in subsequent request messages. Keep reasoning_content for DeepSeek
            # (DeepSeek requires it to be passed back).
            cleaned = {}
            for k, v in message.items():
                if k in ("role", "content", "tool_calls", "refusal", "reasoning_content"):
                    cleaned[k] = v
            # Remove null tool_calls (some providers reject it)
            if cleaned.get("tool_calls") is None:
                cleaned.pop("tool_calls", None)
            self._openai_messages.append(cleaned)
                ...
                if batch["concurrent"]:
                    async def _run_oai_safe(ct_item: dict) -> tuple[dict, str]:
                        try:
                            ...
                            err = f"Error executing {ct_item['fn']}: {exc}"
                            print_tool_result(ct_item["fn"], err)
                            return ct_item, err

                    results = await asyncio.gather(*[_run_oai_safe(ct) for ct in batch["items"]], return_exceptions=True)
                    for r in results:
                        if isinstance(r, BaseException):
                            continue  # Should not happen with try/except in _run_oai_safe
                        ct_item, res = r
    ...
    async def _call_openai_stream(self) -> dict:
        async def _do():
            ...
            if self._skill_payload and self._call_supports_extra_body(self._openai_client.chat.completions.create):
                create_params["extra_body"] = {"skills": self._skill_payload}
            ...
            async for chunk in stream:
                ...
                # Capture reasoning_content (e.g. DeepSeek thinking) for message history
