"""Skills system — discover, parse, and execute skills/*/SKILL.md
...
def discover_skills() -> list[SkillDefinition]:
    ...
    if skills_dir.is_dir():
        for skill_dir in sorted(skills_dir.iterdir(), key=lambda p: p.name):
            if skill_dir.is_dir():
                _load_skills_from_dir(skill_dir, "project", skills)
...
def _load_skills_from_dir(
    skill_dir: Path, source: str, skills: dict[str, SkillDefinition]
) -> None:
...
def _parse_skill_file(
    ...
    try:
        ...
        user_invocable_raw = meta.get("user-invocable", meta.get("user_invocable", "true"))
        user_invocable = str(user_invocable_raw).lower() != "false"
...
def build_skill_payload() -> list[dict]:
    """Build a structured skill list for model request payloads.

    This is intentionally concise metadata, not full prompt bodies.
    Prompt bodies should still be loaded via the skill tool when selected.
    """
    payload: list[dict] = []
    for s in discover_skills():
        payload.append(
